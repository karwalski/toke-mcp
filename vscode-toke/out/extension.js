 ...
 */
function findCompanion(tkUri) {
    const tkPath = tkUri.fsPath;
    if (!tkPath.endsWith(".tk"))
        return undefined;
 ...
 */
async function openCompanionBeside(companionUri) {
    // Check if already open in a visible editor — don't open again
    const alreadyOpen = vscode.window.visibleTextEditors.some((e) => e.document.uri.fsPath === companionUri.fsPath);
    if (alreadyOpen)
        return;
    await vscode.commands.executeCommand("vscode.open", companionUri, { viewColumn: vscode.ViewColumn.Beside, preview: true });
}
/**
...
function activate(context) {
    ...
    companionStatusItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 99);
    ...
    context.subscriptions.push(vscode.commands.registerCommand("toke.openCompanion", () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor || editor.document.languageId !== "toke") {
        ...
        if (companion) {
            ...
            vscode.window.showInformationMessage("No companion file found. Expected: .tkc.md, .tkc.yaml, or .tkc.json alongside the .tk file.");
        }
    }));
    // ── Auto-open companion on .tk file open ──
    context.subscriptions.push(vscode.workspace.onDidOpenTextDocument((doc) => {
        if (doc.languageId !== "toke")
            return;
    ...
    context.subscriptions.push(watcher.onDidCreate(() => updateCompanionStatus()), watcher.onDidDelete(() => updateCompanionStatus()), watcher);
    // ── LSP client ──
    const lspConfig = vscode.workspace.getConfiguration("toke.lsp");
    const lspEnabled = lspConfig.get("enabled", true);
    const lspPath = lspConfig.get("path", "toke-lsp");
    if (!lspEnabled) {
        return;
    }
    const serverOptions = {
    ...
    client = new node_1.LanguageClient("tokeLsp", "Toke Language Server", serverOptions, clientOptions);
    client.start().catch((err) => {
        const msg = err instanceof Error ? err.message : String(err);
        if (!msg.includes("ENOENT")) {
            vscode.window.showWarningMessage(`Toke LSP failed to start: ${msg}`);
        }
    });
